...
anglesL = [0,20,0,-120,0,90,0]
anglesR = [0,20,0,-120,0,90,0]
ml.send_angles(anglesL,10,_async=1)
...
ml.set_movement_type(3)  #切换为速度环融合规划
ml.set_vr_mode(1)    #将机械臂内部点位缓存窗口修改为2
mr.set_movement_type(3)  #切换为速度环融合规划
...
def capture_mouse_position():
    ...
    while True:
        ...
        st = time.time()
        TI = 6
        ml.send_base_coords(target_l, TI, _async=1)  #安全起见初始速度较慢，最快不要小于3
        mr.send_base_coords(target_r, TI, _async=1)  #安全起见初始速度较慢，最快不要小于3
        # 等待10毫秒
        time.sleep(0.01)
        # 参数说明：
        # 	target：目标位置（1*6坐标）
